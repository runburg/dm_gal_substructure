...
def gaussian_mean(M, r, a=77.4, b=0.87, c=-0.23, fwimp=1):
    ...
    """
    return a + b * np.log(M / 1e5) + c * np.log(r / 50) + np.log(fwimp)
...
def conditional_luminosity_function(lnL, M, r, mean_params={}, sd_params={}, fwimp=1):
    ...
    Returns:
        - value of the clf
    """
    mean = gaussian_mean(M, r, **mean_params, fwimp=fwimp)
...
def p1(F, psi, num=100, R_G=220, M_min=0.01, M_max=1e10, d_solar=8.5, mean_params={}, sd_params={}, fwimp=1, **kwargs):
    ...
    def integrand_2d(M, l):
        ...
        return l**4 * mass_distribution(M, r, **kwargs) / Lsh * conditional_luminosity_function(np.log(Lsh), M, r, mean_params=mean_params, sd_params=sd_params, fwimp=fwimp)
...
def likelihood2d(params, psh, subcounts, fluxes, eg_bg_count=np.array([0]), g_bg_count=np.array([0])):
    from scipy.stats import poisson
    exposure, beg, fwimp = params

    pc = integrate.simps(psh * poisson.pmf(subcounts[np.newaxis, :], exposure * fwimp * fluxes[:, np.newaxis] + beg * eg_bg_count + g_bg_count[np.newaxis, :]), fluxes, axis=0)

    pixel_probs = pc
    if np.any(pixel_probs <=0):
        print('zero prob pixels', np.sum(pixel_probs <= 0), np.sum(pixel_probs == 0))

    S = -2 * np.sum(np.log(pixel_probs, where=(pixel_probs > 0)))
...
def poisson_likelihood2d(params, psh, subcounts, fluxes, eg_bg_count=np.array([0]), g_bg_count=np.array([0])):
    from scipy.stats import poisson
    exposure, beg, fwimp = params
    ...
    pc = poisson.pmf(subcounts, exposure * fwimp * mmean_f + beg * eg_bg_count + g_bg_count)
